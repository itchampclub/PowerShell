#################################################################
#
# You need to go through this script and replace <<DOMAIN>>
#  and <<server>> with your appropriate values.
#
#################################################################
...
$termOU = (gl).Path
$count = 30
$seedDir = "\\<<server>>\users$\TBecker\GitHub\PowerShell"
$seedPST = "\\<<server>>\terminations`$\test.pst"
...
for ($i = 1; $i -le $count; $i++)
{
    ...
    $AccessRule=NEW-OBJECT System.Security.AccessControl.FileSystemAccessRule ("<<DOMAIN>>\$($thisuser.SamAccountName)",”FullControl”,”ContainerInherit, ObjectInherit”,”None”,”Allow”)
