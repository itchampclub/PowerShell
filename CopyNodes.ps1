...
# This script is a big work in progress.
# Download the Solarwinds SDK and run the following or the equivalent on your system
...
$credential1 = $host.ui.PromptForCredential("Need credentials", "Please enter your user name and password for $hostname1", $username1 , "")
$source = Connect-Swis -Credential $credential1 -Hostname $hostname1

# Connect to the target system
$credential2 = $host.ui.PromptForCredential("Need credentials", "Please enter your user name and password for $hostname2", $username2 , "")
...
# Check whether the target has NPM installed
$targetHasNPM = (Get-SwisData $target "SELECT CanInvoke FROM Metadata.Verb WHERE EntityName='Cirrus.Nodes' AND Name='AddNode'") -ne $null
...
$sourceNodes = Get-SwisData $source "SELECT Nodes.Uri, IPAddress, Caption, Nodes.NodeID FROM Orion.Nodes inner join Orion.NodesCustomProperties on Nodes.NodeID = NodesCustomProperties.NodeID WHERE NodesCustomProperties.GlobalView=1"
...
foreach ($sourceNode in $sourceNodes) {
    ...
    # If the target has NPM installed, add the new node to NPM
    if ($targetHasNPM) {
        Invoke-SwisVerb $target Cirrus.Nodes AddNode $newNode.NodeID
    }
}
